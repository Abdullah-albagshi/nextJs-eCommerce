...
>(({ className, ...props }, ref) => {
  ...
  // slidesNodes total number of slides
...
type CarouselThumbsProps = { images: string[] } & React.HTMLAttributes<HTMLDivElement>;

const CarouselThumbs = React.forwardRef<HTMLDivElement, CarouselThumbsProps>(({...props}, ref) => {
  ...
  return (
    <div
      ref={ref}
      className='hidden lg:flex flex-col items-center justify-start space-y-2'
      {...props}
    >
      {props.images.map((slide, index) => (
        <button
          key={index}
          className={cn('w-16 h-16 rounded-md cursor-pointer transition-all duration-300 ',
